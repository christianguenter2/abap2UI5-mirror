CLASS z2ui6_cx_ui5_util_error DEFINITION
  PUBLIC
...
CLASS z2ui6_cx_ui5_util_error IMPLEMENTATION.
  METHOD constructor ##ADT_SUPPRESS_GENERATION.
    ...
    TRY.
      ...
      CATCH cx_root.
        ...
        IF z2ui6_cl_ui5_util_context=>rtti_check_printable( val ) = abap_true.
          lv_text = val.
        ENDIF.
    ENDTRY.
  ...
  METHOD get_text_own_by_x.
    ...
    TRY.
        result = CAST z2ui6_cx_ui5_util_error( val )->get_text_own( ).
    ...
    WHILE lo_x IS BOUND AND lv_count < cv_chain_max.
      ...
      IF lv_text IS NOT INITIAL AND lv_text <> lv_last.
        result = COND #( WHEN result IS INITIAL
                         THEN lv_text
                         ELSE result && z2ui6_cl_ui5_util_context=>cv_char_util_newline && lv_text ).
        lv_last = lv_text.
      ENDIF.
      lo_x = lo_x->previous.
    ENDWHILE.
  ...
  METHOD get_text_full_entry.
    ...
    result = |[{ index }] { z2ui6_cl_ui5_util_context=>rtti_get_classname_by_ref( val ) }|.
    ...
    DATA(lv_position) = z2ui6_cl_ui5_util_context=>error_get_source_position( val ).
    ...
    TRY.
        DATA(lx_own) = CAST z2ui6_cx_ui5_util_error( val ).
        ...
        IF lx_own->ms_error-uuid IS INITIAL.
          lx_own->ms_error-uuid = z2ui6_cl_ui5_util_context=>uuid_get_c32( ).
        ENDIF.
        result = result && lv_nl && |    id       : { lx_own->ms_error-uuid }|.
      CATCH cx_root ##NO_HANDLER.
    ENDTRY.

    DATA(lt_attri) = z2ui6_cl_ui5_util_context=>error_get_attributes( val ).
