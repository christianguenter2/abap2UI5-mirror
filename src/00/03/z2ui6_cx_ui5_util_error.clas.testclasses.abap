...
CLASS ltcl_unit_test IMPLEMENTATION.
  ...
  METHOD test_raise_with_prev.
    ...
    DATA(lx_prev) = NEW z2ui6_cx_ui5_util_error( val = `previous error` ).
  ...
  METHOD test_uuid_populated.
    ...
    TRY.
      ...
      CATCH z2ui6_cx_ui5_util_error INTO DATA(lx).
        ...
        DATA(lv_text) = z2ui6_cx_ui5_util_error=>get_text_full( lx ).
  ...
  METHOD test_chain_texts.
    ...
    DATA(lx_inner) = NEW z2ui6_cx_ui5_util_error( val = `inner` ).
    DATA(lx_middle) = NEW z2ui6_cx_ui5_util_error( val   = `middle`
                                                previous = lx_inner ).
    DATA(lx_outer) = NEW z2ui6_cx_ui5_util_error( val   = `outer`
                                               previous = lx_middle ).
  ...
  METHOD test_cause_kept_by_val.
    ...
    DATA(lx_inner) = NEW z2ui6_cx_ui5_util_error( val = `root cause` ).
    DATA(lx_middle) = NEW z2ui6_cx_ui5_util_error( val   = `middle layer`
                                                previous = lx_inner ).
  ...
  METHOD test_no_duplicate_text.
    ...
    DATA(lx_inner) = NEW z2ui6_cx_ui5_util_error( val = `same text` ).
    DATA(lx_outer) = NEW z2ui6_cx_ui5_util_error( val   = `same text`
                                               previous = lx_inner ).
  ...
  METHOD test_text_full_chain.
    ...
    DATA(lx_inner) = NEW z2ui6_cx_ui5_util_error( val = `root cause` ).
    DATA(lx_outer) = NEW z2ui6_cx_ui5_util_error( val   = `outer problem`
                                               previous = lx_inner ).

    DATA(lv_text) = z2ui6_cx_ui5_util_error=>get_text_full( lx_outer ).
  ...
  METHOD test_text_full_any_cx.
    ...
    DATA(lv_text) = z2ui6_cx_ui5_util_error=>get_text_full( lx_root ).
  ...
  METHOD test_text_full_unbound.
    ...
    cl_abap_unit_assert=>assert_equals( exp = `UNKNOWN_ERROR`
                                        act = z2ui6_cx_ui5_util_error=>get_text_full( lx_unbound ) ).
  ...
  METHOD test_chain_bounded.
    ...
    DATA(lx) = NEW z2ui6_cx_ui5_util_error( val = `level 0` ).

    DO 30 TIMES.
      lx = NEW z2ui6_cx_ui5_util_error( val   = |level { sy-index }|
                                     previous = lx ).
    ENDDO.

    cl_abap_unit_assert=>assert_true(
      xsdbool( z2ui6_cx_ui5_util_error=>get_text_full( lx ) CS `chain truncated` ) ).
    cl_abap_unit_assert=>assert_not_initial( lx->get_text( ) ).
