CLASS z5ui5_cl_pop_bal DEFINITION
  PUBLIC FINAL
  CREATE PUBLIC.
  ...
  PUBLIC SECTION.
    ...
    CLASS-METHODS factory
      ...
      RETURNING
        VALUE(r_result) TYPE REF TO z5ui5_cl_pop_bal.
...
CLASS z5ui5_cl_pop_bal IMPLEMENTATION.
  METHOD factory.
    ...
    "read messages..
    DATA(lt_msg) = z5ui5_cl_util=>msg_get( i_messages ).
    LOOP AT lt_msg REFERENCE INTO DATA(lr_row).

      DATA(ls_row) = VALUE ty_s_msg( ).
      ls_row-type     = z5ui5_cl_util=>ui5_get_msg_type( lr_row->type ).
      ls_row-title    = lr_row->text.
*      lr_row->title = `title`.
*      lr_row->message = `message`.
      ls_row-subtitle = |{ lr_row->id } { lr_row->no }|.
      ls_row-date = z5ui5_cl_util=>time_get_date_by_stampl( lr_row->timestampl ).
      ls_row-time = z5ui5_cl_util=>time_get_time_by_stampl( lr_row->timestampl ).
*      lr_row->group = `001`.
