CLASS z5ui5_cl_pop_table DEFINITION
  PUBLIC FINAL
  CREATE PROTECTED.
  ...
  PUBLIC SECTION.
    ...
    CLASS-METHODS factory
      ...
      RETURNING
        VALUE(r_result) TYPE REF TO z5ui5_cl_pop_table.
...
CLASS z5ui5_cl_pop_table IMPLEMENTATION.
  METHOD display.
    ...
    DATA(popup) = z5ui5_cl_xml_view=>factory_popup( )->dialog( afterclose = client->_event( 'BUTTON_CONFIRM' )
                                                               stretch    = abap_true
                                                               title      = title
*                                                               icon       = 'sap-icon://edit'
          )->content( ).
    ...
    DATA(lt_comp) = z5ui5_cl_util=>rtti_get_t_attri_by_any( <tab_out> ).
