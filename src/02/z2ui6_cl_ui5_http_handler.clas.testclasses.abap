...
CLASS ltcl_test_http_handler IMPLEMENTATION.
  ...
  METHOD test_http_get_status.
    ...
    DATA ls_result TYPE z2ui6_cl_ui5_http_handler=>ty_s_http_res.

    ls_result = z2ui6_cl_ui5_http_handler=>_http_get( ).
  ...
  METHOD test_http_post_ok.
    ...
    DATA ls_req TYPE z2ui6_cl_ui5_http_handler=>ty_s_http_req.
    DATA ls_result TYPE z2ui6_cl_ui5_http_handler=>ty_s_http_res.

    ls_req-method = `POST`.
    ls_req-body = `{"value":{"S_FRONT":{"ORIGIN":"O","PATHNAME":"/p","SEARCH":""}}}`.

    ls_result = z2ui6_cl_ui5_http_handler=>_http_post( ls_req ).
  ...
  METHOD test_http_post_error.
    ...
    TRY.
        z2ui6_cl_ui5_http_handler=>_http_post( ls_req ).
      CATCH cx_root.
        lv_raised = abap_true.
    ENDTRY.
  ...
  METHOD test_main_post_no_app.
    ...
    ls_result = z2ui6_cl_ui5_http_handler=>_main( ls_req ).
  ...
  METHOD test_csrf_inactive.
    ...
    DATA(lv_rejected) = z2ui6_cl_ui5_http_handler=>_check_csrf_rejected(
                            active  = abap_false
  ...
  METHOD test_preload_escaping.
    ...
    DATA(lv_preload) = z2ui6_cl_ui5f_preload=>get( styles_css = lv_css
                                                  custom_js   = `` ).
  ...
  METHOD test_preload_literals.
    ...
    DATA(lv_preload) = z2ui6_cl_ui5f_preload=>get( styles_css = `.a { content: 'x'; }`
                                                  custom_js   = `` ).
...
CLASS ltcl_http_mock DEFINITION INHERITING FROM z2ui6_cl_ui5_util_http
  FINAL FOR TESTING.

  PUBLIC SECTION.
    DATA ms_req_info   TYPE ty_s_http_req.
    DATA mt_req_header TYPE z2ui6_if_client=>ty_t_name_value.
    DATA mt_res_header TYPE z2ui6_if_client=>ty_t_name_value.
...
CLASS ltcl_exit_stub IMPLEMENTATION.
  ...
  METHOD z2ui6_if_ui5_exit~set_config_http_get.
  ...
  METHOD z2ui6_if_ui5_exit~set_config_http_post.
...
CLASS ltcl_exit_injector DEFINITION INHERITING FROM z2ui6_cl_ui5_user_exit
  FINAL FOR TESTING.

  PUBLIC SECTION.
    CLASS-METHODS inject
      IMPORTING
        ii_exit TYPE REF TO z2ui6_if_ui5_exit.
...
CLASS ltcl_test_http_response DEFINITION FINAL
  ...
  PRIVATE SECTION.
    DATA mo_handler TYPE REF TO z2ui6_cl_ui5_http_handler.
    ...
    METHODS shell_for_config
      IMPORTING
        is_config     TYPE z2ui6_if_ui5_exit=>ty_s_http_config
      RETURNING
        VALUE(result) TYPE string.
...
CLASS z2ui6_cl_ui5_http_handler DEFINITION LOCAL FRIENDS ltcl_test_http_response.
...
CLASS ltcl_test_http_response IMPLEMENTATION.
  ...
  METHOD caches_clear.
    CLEAR z2ui6_cl_ui5_http_handler=>ss_config_http_get.
    CLEAR z2ui6_cl_ui5_http_handler=>sv_config_http_get_set.
    CLEAR z2ui6_cl_ui5_http_handler=>sv_get_cache_key.
    CLEAR z2ui6_cl_ui5_http_handler=>sv_get_cache_body.
    CLEAR z2ui6_cl_ui5_http_handler=>sv_get_etag.
    CLEAR z2ui6_cl_ui5_http_handler=>sv_if_none_match.
  ENDMETHOD.
  ...
  METHOD shell_for_config.
    z2ui6_cl_ui5_http_handler=>ss_config_http_get     = is_config.
    z2ui6_cl_ui5_http_handler=>sv_config_http_get_set = abap_true.
    DATA(ls_res) = z2ui6_cl_ui5_http_handler=>_http_get( ).
    result = ls_res-body.
  ENDMETHOD.
  ...
  METHOD test_cache_key_covers_inputs.
    ...
    DATA(ls_base) = VALUE z2ui6_if_ui5_exit=>ty_s_http_config(
        theme                   = `sap_horizon`
    ...
    DATA(lv_key_base)  = z2ui6_cl_ui5_http_handler=>sv_get_cache_key.
    ...
    cl_abap_unit_assert=>assert_differs( exp = lv_key_base
                                         act = z2ui6_cl_ui5_http_handler=>sv_get_cache_key ).
  ...
  METHOD test_cache_hit_same_config.
    ...
    DATA(ls_config) = VALUE z2ui6_if_ui5_exit=>ty_s_http_config(
        theme = `sap_horizon`
        src   = `https://sdk.example/sap-ui-core.js` ).
    ...
    z2ui6_cl_ui5_http_handler=>sv_get_cache_body = `CACHED_SENTINEL`.
    DATA(lv_body) = shell_for_config( ls_config ).
  ...
  METHOD test_get_304_on_if_none_match.
    ...
    mo_handler->ms_res = VALUE #( body          = `<html>shell</html>`
                                  status_code   = 200
                                  status_reason = `OK` ).
    z2ui6_cl_ui5_http_handler=>sv_get_etag = `"1.0-11-22-33"`.
  ...
  METHOD test_etag_match_rules.
    ...
    cl_abap_unit_assert=>assert_true( z2ui6_cl_ui5_http_handler=>_check_etag_match(
    ...
    cl_abap_unit_assert=>assert_false( z2ui6_cl_ui5_http_handler=>_check_etag_match(
  ...
  METHOD test_etag_equal_length.
    ...
    DATA(lv_tag_a) = z2ui6_cl_ui5_http_handler=>_get_etag( `<html>shell content A</html>` ).
    DATA(lv_tag_b) = z2ui6_cl_ui5_http_handler=>_get_etag( `<html>shell content B</html>` ).
    ...
    cl_abap_unit_assert=>assert_equals(
        exp = lv_tag_a
        act = z2ui6_cl_ui5_http_handler=>_get_etag( `<html>shell content A</html>` ) ).
  ...
  METHOD test_etag_position_mix.
    ...
    DATA(lv_tag_a) = z2ui6_cl_ui5_http_handler=>_get_etag( `abcdefgh` ).
    DATA(lv_tag_b) = z2ui6_cl_ui5_http_handler=>_get_etag( `hgfedcba` ).
  ...
  METHOD test_etag_follows_cache_key.
    ...
    DATA(ls_config_a) = VALUE z2ui6_if_ui5_exit=>ty_s_http_config(
        theme = `sap_horizon`
        src   = `https://sdk.example/sap-ui-core.js` ).
    shell_for_config( ls_config_a ).
    DATA(lv_tag_a) = z2ui6_cl_ui5_http_handler=>sv_get_etag.

    cl_abap_unit_assert=>assert_not_initial( lv_tag_a ).
    cl_abap_unit_assert=>assert_char_cp( act = lv_tag_a
                                         exp = |*-{ z2ui6_cl_ui5f_preload=>build_hash }-*| ).

    DATA(ls_config_b) = ls_config_a.
    ls_config_b-theme = `sap_fiori_3`.
    shell_for_config( ls_config_b ).
    cl_abap_unit_assert=>assert_differs( exp = lv_tag_a
                                         act = z2ui6_cl_ui5_http_handler=>sv_get_etag ).
  ...
  METHOD test_get_304_skips_build.
    ...
    DATA(ls_config) = VALUE z2ui6_if_ui5_exit=>ty_s_http_config(
        ...
        src   = `https://sdk.example/sap-ui-core.js` ).
    shell_for_config( ls_config ).
    DATA(lv_tag) = z2ui6_cl_ui5_http_handler=>sv_get_etag.
    cl_abap_unit_assert=>assert_not_initial( lv_tag ).

    caches_clear( ).
    z2ui6_cl_ui5_http_handler=>sv_if_none_match       = lv_tag.
    z2ui6_cl_ui5_http_handler=>ss_config_http_get     = ls_config.
    z2ui6_cl_ui5_http_handler=>sv_config_http_get_set = abap_true.
    ...
    cl_abap_unit_assert=>assert_initial( z2ui6_cl_ui5_http_handler=>sv_get_cache_body ).
    ...
    " request
    cl_abap_unit_assert=>assert_initial( z2ui6_cl_ui5_http_handler=>sv_if_none_match ).
  ...
  METHOD test_get_stale_tag_builds.
    ...
    DATA(ls_config) = VALUE z2ui6_if_ui5_exit=>ty_s_http_config(
        theme = `sap_horizon`
        src   = `https://sdk.example/sap-ui-core.js` ).
    z2ui6_cl_ui5_http_handler=>sv_if_none_match       = `"1.0-stale-tag"`.
    z2ui6_cl_ui5_http_handler=>ss_config_http_get     = ls_config.
    z2ui6_cl_ui5_http_handler=>sv_config_http_get_set = abap_true.
    ...
    cl_abap_unit_assert=>assert_not_initial( z2ui6_cl_ui5_http_handler=>sv_get_cache_body ).
    cl_abap_unit_assert=>assert_not_initial( z2ui6_cl_ui5_http_handler=>sv_get_etag ).
    cl_abap_unit_assert=>assert_initial( z2ui6_cl_ui5_http_handler=>sv_if_none_match ).
