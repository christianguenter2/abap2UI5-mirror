...
CLASS ltcl_test IMPLEMENTATION.
  ...
  METHOD test_factory.
    ...
    DATA(lt_filter) = VALUE z2ui6_cl_ui5_util_context=>ty_t_filter_multi(
      ( name = `CARRID` t_range = VALUE #( ( sign = `I` option = `EQ` low = `AA` ) ) )
      ( name = `CONNID` ) ).

    DATA(lo_pop) = z2ui6_cl_pop_get_range_m=>factory( lt_filter ).
...
CLASS ltcl_test_roundtrip DEFINITION FINAL
  ...
  PRIVATE SECTION.
    ...
    METHODS popup_create
      RETURNING
        VALUE(ro_pop) TYPE REF TO z2ui6_cl_pop_get_range_m.
...
CLASS ltcl_test_roundtrip IMPLEMENTATION.
  ...
  METHOD popup_create.
    ...
    ro_pop = z2ui6_cl_pop_get_range_m=>factory( VALUE #(
        ( name    = `MATNR`
  ...
  METHOD test_list_open_calls_pop.
    ...
    DATA(lo_range_pop) = CAST z2ui6_cl_pop_get_range( mo_action->ms_next-o_app_call ).
