...
CLASS z2ui6_cl_pop_to_confirm DEFINITION LOCAL FRIENDS ltcl_test ltcl_test_events.
...
CLASS ltcl_test IMPLEMENTATION.
  ...
  METHOD test_factory.
    ...
    DATA(lo_pop) = z2ui6_cl_pop_to_confirm=>factory( `Are you sure?` ).
  ...
  METHOD test_factory_defaults.
    ...
    DATA(lo_pop) = z2ui6_cl_pop_to_confirm=>factory( `Delete?` ).
    ...
    cl_abap_unit_assert=>assert_equals( exp = z2ui6_cl_pop_to_confirm=>cs_event-confirmed
                                        act = lo_pop->event_confirm ).
    cl_abap_unit_assert=>assert_equals( exp = z2ui6_cl_pop_to_confirm=>cs_event-canceled
  ...
  METHOD test_factory_custom.
    ...
    DATA(lo_pop) = z2ui6_cl_pop_to_confirm=>factory(
  ...
  METHOD test_result_initial.
    ...
    DATA(lo_pop) = z2ui6_cl_pop_to_confirm=>factory( `Test?` ).
    cl_abap_unit_assert=>assert_false( lo_pop->result( ) ).
  ...
  METHOD test_constants.
    ...
    cl_abap_unit_assert=>assert_not_initial( z2ui6_cl_pop_to_confirm=>cs_event-confirmed ).
    cl_abap_unit_assert=>assert_not_initial( z2ui6_cl_pop_to_confirm=>cs_event-canceled ).
...
CLASS ltcl_test_events IMPLEMENTATION.
  ...
  METHOD test_default_events_differ.
    ...
    cl_abap_unit_assert=>assert_differs(
      act = z2ui6_cl_pop_to_confirm=>cs_event-confirmed
      exp = z2ui6_cl_pop_to_confirm=>cs_event-canceled ).
...
CLASS ltcl_test_roundtrip DEFINITION FINAL
  ...
  PRIVATE SECTION.
    ...
    DATA mi_client TYPE REF TO z2ui6_if_client.
...
CLASS ltcl_test_roundtrip IMPLEMENTATION.
  ...
  METHOD popup_displayed_xml.
    ...
    result = VALUE #( mo_action->ms_next-t_action_front[
                          slot   = z2ui6_if_client=>cs_view-popup
                          method = z2ui6_if_ui5_types=>cs_slot_action-display ]-xml OPTIONAL ).
  ...
  METHOD client_create.
    ...
    mo_action = NEW #( NEW z2ui6_cl_ui5_handler( `` ) ).
    mo_action->mo_app->mo_app = io_app.
    mi_client = NEW z2ui6_cl_ui5_client( mo_action ).
  ...
  METHOD test_init_displays_popup.
    ...
    DATA(lo_pop) = z2ui6_cl_pop_to_confirm=>factory( i_question_text = `Are you sure?`
                                                     i_title         = `My Title` ).
    client_create( lo_pop ).

    lo_pop->z2ui6_if_app~main( mi_client ).
  ...
  METHOD test_confirm.
    ...
    DATA(lo_pop) = z2ui6_cl_pop_to_confirm=>factory( `Sure?` ).
  ...
  METHOD test_custom_event_confirm.
    ...
    DATA(lo_pop) = z2ui6_cl_pop_to_confirm=>factory( i_question_text = `Sure?`
                                                     i_event_confirm = `MY_CONFIRM` ).
    roundtrip_event( io_app   = lo_pop
                     iv_event = `BUTTON_CONFIRM` ).
