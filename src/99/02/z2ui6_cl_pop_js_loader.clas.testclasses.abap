...
CLASS ltcl_test IMPLEMENTATION.
  ...
  METHOD test_factory.
    DATA(lo_pop) = z2ui6_cl_pop_js_loader=>factory(
  ...
  METHOD test_factory_open_ui5.
    DATA(lo_pop) = z2ui6_cl_pop_js_loader=>factory_check_open_ui5( ).

    cl_abap_unit_assert=>assert_bound( lo_pop ).
  ENDMETHOD.

  METHOD test_result_initial.
    DATA(lo_pop) = z2ui6_cl_pop_js_loader=>factory( `alert(1);` ).
...
CLASS ltcl_test_roundtrip IMPLEMENTATION.
  ...
  METHOD test_init_displays_script.
    ...
    DATA(lo_pop) = z2ui6_cl_pop_js_loader=>factory( `console.log('x');` ).
