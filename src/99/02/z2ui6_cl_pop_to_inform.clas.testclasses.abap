CLASS ltcl_test DEFINITION DEFERRED.
CLASS z2ui6_cl_pop_to_inform DEFINITION LOCAL FRIENDS ltcl_test.

CLASS ltcl_test DEFINITION FINAL
  FOR TESTING RISK LEVEL HARMLESS DURATION SHORT.

  PRIVATE SECTION.

    METHODS test_factory          FOR TESTING RAISING cx_static_check.
    METHODS test_factory_defaults FOR TESTING RAISING cx_static_check.
    METHODS test_factory_custom   FOR TESTING RAISING cx_static_check.

ENDCLASS.


CLASS ltcl_test IMPLEMENTATION.

  METHOD test_factory.

    DATA(lo_pop) = z2ui6_cl_pop_to_inform=>factory( `Info message` ).
    cl_abap_unit_assert=>assert_bound( lo_pop ).

  ENDMETHOD.

  METHOD test_factory_defaults.

    DATA(lo_pop) = z2ui6_cl_pop_to_inform=>factory( `Hello` ).

    cl_abap_unit_assert=>assert_equals( exp = `Hello`
                                        act = lo_pop->question_text ).
    cl_abap_unit_assert=>assert_equals( exp = `Information`
                                        act = lo_pop->title ).
    cl_abap_unit_assert=>assert_equals( exp = `sap-icon://information`
                                        act = lo_pop->icon ).
    cl_abap_unit_assert=>assert_equals( exp = `OK`
                                        act = lo_pop->button_text_confirm ).

  ENDMETHOD.

  METHOD test_factory_custom.

    DATA(lo_pop) = z2ui6_cl_pop_to_inform=>factory(
      i_text        = `Custom Info`
      i_title       = `My Title`
      i_icon        = `sap-icon://hint`
      i_button_text = `Close` ).

    cl_abap_unit_assert=>assert_equals( exp = `Custom Info`
                                        act = lo_pop->question_text ).
    cl_abap_unit_assert=>assert_equals( exp = `My Title`
                                        act = lo_pop->title ).
    cl_abap_unit_assert=>assert_equals( exp = `sap-icon://hint`
                                        act = lo_pop->icon ).
    cl_abap_unit_assert=>assert_equals( exp = `Close`
                                        act = lo_pop->button_text_confirm ).

  ENDMETHOD.

ENDCLASS.


CLASS ltcl_test_roundtrip DEFINITION FINAL
  FOR TESTING RISK LEVEL HARMLESS DURATION SHORT.

  PRIVATE SECTION.
    DATA mo_action TYPE REF TO z2ui6_cl_ui5_action.
    DATA mi_client TYPE REF TO z2ui6_if_client.

    METHODS popup_displayed_xml
      RETURNING
        VALUE(result) TYPE string.

    METHODS popup_destroy_queued
      RETURNING
        VALUE(result) TYPE abap_bool.

    METHODS client_create
      IMPORTING
        io_app TYPE REF TO z2ui6_if_app.

    METHODS roundtrip_event
      IMPORTING
        io_app   TYPE REF TO z2ui6_if_app
        iv_event TYPE string.

    METHODS test_init_displays_popup FOR TESTING RAISING cx_static_check.
    METHODS test_confirm_closes      FOR TESTING RAISING cx_static_check.

ENDCLASS.


CLASS ltcl_test_roundtrip IMPLEMENTATION.

  METHOD popup_displayed_xml.

    result = VALUE #( mo_action->ms_next-t_action_front[
                          slot   = z2ui6_if_client=>cs_view-popup
                          method = z2ui6_if_ui5_types=>cs_slot_action-display ]-xml OPTIONAL ).

  ENDMETHOD.


  METHOD popup_destroy_queued.

    result = xsdbool( line_exists( mo_action->ms_next-t_action_front[
                          slot   = z2ui6_if_client=>cs_view-popup
                          method = z2ui6_if_ui5_types=>cs_slot_action-destroy ] ) ).

  ENDMETHOD.


  METHOD client_create.

    mo_action = NEW #( NEW z2ui6_cl_ui5_handler( `` ) ).
    mo_action->mo_app->mo_app = io_app.
    mi_client = NEW z2ui6_cl_ui5_client( mo_action ).

  ENDMETHOD.

  METHOD roundtrip_event.

    client_create( io_app ).
    mo_action->mo_app->mv_check_initialized = abap_true.
    mo_action->ms_actual-event = iv_event.
    io_app->main( mi_client ).

  ENDMETHOD.

  METHOD test_init_displays_popup.

    DATA(lo_pop) = z2ui6_cl_pop_to_inform=>factory( i_text  = `All done!`
                                                    i_title = `Info Title` ).
    client_create( lo_pop ).

    lo_pop->z2ui6_if_app~main( mi_client ).

    DATA(lv_xml) = popup_displayed_xml( ).
    cl_abap_unit_assert=>assert_true( xsdbool( lv_xml CS `All done!` ) ).
    cl_abap_unit_assert=>assert_true( xsdbool( lv_xml CS `Info Title` ) ).

  ENDMETHOD.

  METHOD test_confirm_closes.

    DATA(lo_pop) = z2ui6_cl_pop_to_inform=>factory( `All done!` ).
    roundtrip_event( io_app   = lo_pop
                     iv_event = `BUTTON_CONFIRM` ).

    cl_abap_unit_assert=>assert_true( popup_destroy_queued( ) ).
    cl_abap_unit_assert=>assert_bound( mo_action->ms_next-o_app_leave ).

  ENDMETHOD.

ENDCLASS.
