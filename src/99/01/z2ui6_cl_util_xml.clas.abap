CLASS z2ui6_cl_util_xml DEFINITION
  PUBLIC FINAL
  CREATE PROTECTED.
  ...
  PUBLIC SECTION.
    ...
    CLASS-METHODS factory
      RETURNING
        VALUE(result) TYPE REF TO z2ui6_cl_util_xml.
    ...
    METHODS __
      IMPORTING
        ...
        p             TYPE z2ui6_cl_ui5_util_context=>ty_t_name_value OPTIONAL
      RETURNING
        VALUE(result) TYPE REF TO z2ui6_cl_util_xml.
  ...
  PROTECTED SECTION.
    ...
    DATA mt_prop   TYPE SORTED TABLE OF z2ui6_cl_ui5_util_context=>ty_s_name_value WITH NON-UNIQUE KEY n.

    DATA mo_root   TYPE REF TO z2ui6_cl_util_xml.
    DATA mo_previous TYPE REF TO z2ui6_cl_util_xml.
    DATA mo_parent TYPE REF TO z2ui6_cl_util_xml.
    DATA mt_child  TYPE STANDARD TABLE OF REF TO z2ui6_cl_util_xml WITH EMPTY KEY.
...
CLASS z2ui6_cl_util_xml IMPLEMENTATION.
  ...
  METHOD __.
    ...
    DATA(lo_child) = NEW z2ui6_cl_util_xml( ).
    lo_child->mv_name   = n.
    lo_child->mv_ns     = ns.
    lo_child->mt_prop   = p.
    IF a IS NOT INITIAL.
  ...
  METHOD xml_get_parts.
    ...
    IF mv_name IS INITIAL.
      LOOP AT mt_child INTO DATA(lr_root).
        CAST z2ui6_cl_util_xml( lr_root )->xml_get_parts( CHANGING ct_parts = ct_parts ).
      ENDLOOP.
      RETURN.
    ENDIF.
    ...
    DATA(lv_tmp3) = REDUCE string( INIT val = `` FOR row IN mt_prop WHERE ( v <> `` ) "#EC CI_SORTSEQ
                          NEXT val = |{ val } { row-n }="{ escape( val    = COND string( WHEN row-v = abap_true
                                                                                         THEN `true`
                                                                                         ELSE row-v )
                                                                   format = z2ui6_cl_ui5_util_context=>cv_format_e_xml_attr ) }"| ).
    ...
    LOOP AT mt_child INTO DATA(lr_child).
      CAST z2ui6_cl_util_xml( lr_child )->xml_get_parts( CHANGING ct_parts = ct_parts ).
    ENDLOOP.
  ...
  METHOD xml_get_parts_indent.
    ...
    IF mv_name IS INITIAL.
      LOOP AT mt_child INTO DATA(lr_root).
        CAST z2ui6_cl_util_xml( lr_root )->xml_get_parts_indent( EXPORTING iv_depth = iv_depth
    ...
    LOOP AT mt_child INTO DATA(lr_child).
      CAST z2ui6_cl_util_xml( lr_child )->xml_get_parts_indent( EXPORTING iv_depth = iv_depth + 1
                                                                 CHANGING ct_parts = ct_parts ).
    ENDLOOP.
