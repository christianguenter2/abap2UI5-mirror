...
CLASS ltcl_test IMPLEMENTATION.
  ...
  METHOD test_save_returns_id.
    ...
    DATA(lv_id) = z2ui6_cl_util_db=>save( uname        = `TESTUSER`
                                          handle       = `HANDLE_SAVE`
                                          data         = ls_data
  ...
  METHOD test_save_load_by_id.
    ...
    z2ui6_cl_util_db=>load_by_id( EXPORTING id     = lv_id
  ...
  METHOD test_save_load_by_handle.
    ...
    z2ui6_cl_util_db=>save( uname        = `TESTUSER`
                            handle       = `HANDLE_A`
                            handle2      = `HANDLE_B`
                            data         = ls_data
                            check_commit = abap_false ).

    z2ui6_cl_util_db=>load_by_handle( EXPORTING uname   = `TESTUSER`
                                                handle  = `HANDLE_A`
                                                handle2 = `HANDLE_B`
                                      IMPORTING result  = ls_loaded ).
  ...
  METHOD test_save_updates_entry.
    ...
    DATA(lv_id_first) = z2ui6_cl_util_db=>save( uname        = `TESTUSER`
                                                handle       = `HANDLE_UPDATE`
                                                data         = VALUE ty_s_data( name = `first` )
                                                check_commit = abap_false ).

    DATA(lv_id_second) = z2ui6_cl_util_db=>save( uname        = `TESTUSER`
                                                 handle       = `HANDLE_UPDATE`
  ...
  METHOD test_load_multi_by_handle.
    ...
    DATA(lt_result) = z2ui6_cl_util_db=>load_multi_by_handle( uname  = `TESTUSER`
                                                              handle = `HANDLE_MULTI` ).
  ...
  METHOD test_delete_by_handle.
    ...
    z2ui6_cl_util_db=>delete_by_handle( uname        = `TESTUSER`
                                        handle       = `HANDLE_DELETE`
                                        check_commit = abap_false ).
    ...
    TRY.
      ...
      CATCH z2ui6_cx_util_error ##NO_HANDLER.
    ENDTRY.
  ...
  METHOD test_load_by_id_not_found.
    ...
    TRY.
        DATA ls_loaded TYPE ty_s_data.
        z2ui6_cl_util_db=>load_by_id( EXPORTING id     = `DOES_NOT_EXIST`
                                      IMPORTING result = ls_loaded ).
