CLASS z2ui6_cl_util_ext DEFINITION
  PUBLIC
  CREATE PUBLIC.
  ...
  PUBLIC SECTION.
    ...
    CLASS-METHODS bal_create
      IMPORTING
        ...
        subobject TYPE clike
        id        TYPE clike
        t_log     TYPE z2ui6_cl_util=>ty_t_msg.
...
CLASS z2ui6_cl_util_ext IMPLEMENTATION.
...
* fallback to RTTI, KEY field does not exist in S/4 2020
    ...
    DATA(lt_comp)  =  z2ui6_cl_util=>rtti_get_t_attri_by_any( tabname ).
...
*    ENDLOOP.
  ...
  METHOD rtti_get_t_dfies_by_table_name.
    ...
    IF z2ui6_cl_util=>context_check_abap_cloud( ) IS NOT INITIAL.
  ...
  METHOD rtti_get_table_desrc.
    ...
    IF z2ui6_cl_util=>context_check_abap_cloud( ).
...
*    set_row_id( ).
  ...
  METHOD _get_e071k_tabkey.
    ...
    LOOP AT dfies INTO DATA(s_dfies) WHERE keyflag = abap_true.
      ...
      IF lv_type NA 'CDNT'.
        ...
        RETURN.
      ELSE.
        lv_field_len = cl_abap_typedescr=>describe_by_data( <value> )->length / z2ui6_cl_util=>cv_char_util_charsize.
      ENDIF.
  ...
  METHOD _set_e071k.
    ...
    DATA(t_comp) = z2ui6_cl_util=>rtti_get_t_attri_by_table_name( 'E071K' ).
...
*    ENDIF.
  ...
  METHOD _set_e071.
    ...
    DATA(t_comp) = z2ui6_cl_util=>rtti_get_t_attri_by_table_name( 'E071' ).
  ...
  METHOD _read_e070.
    ...
    TRY.
        DATA(t_comp) = z2ui6_cl_util=>rtti_get_t_attri_by_table_name( table_name ).
...
*    ENDLOOP.
      ...
      TRY.
          ...
          LOOP AT lt_logs INTO DATA(lo_log).
            ...
            LOOP AT lt_items INTO DATA(ls_item).
              ...
              DATA(ls_msg) = VALUE z2ui6_cl_util=>ty_s_msg( ).
  ...
  METHOD tr_get_user_requests.
    ...
    TRY.
        ...
        DATA(lt_comp) = z2ui6_cl_util=>rtti_get_t_attri_by_table_name( lv_tab1 ).
  ...
  METHOD tr_copy_objects.
    ...
    TRY.
      ...
      CATCH z2ui6_cx_util_error INTO DATA(lx_known).
  ...
  METHOD lock_get_owner.
    ...
    TRY.
        ...
        DATA(lv_name) = z2ui6_cl_util=>c_trim_upper( val ).
  ...
  METHOD lock_get_dequeue_by_enqueue.
    ...
    result = replace( val  = z2ui6_cl_util=>c_trim_upper( val )
                      sub  = `ENQUEUE_`
                      with = `DEQUEUE_` ).
  ...
  METHOD lock_read.
    ...
    TRY.
        ...
        IF client IS INITIAL.
          lv_client = z2ui6_cl_util=>context_get_sy( )-mandt.
        ELSE.
          lv_client = client.
        ENDIF.
        lv_name  = lock_object.
        ...
        IF sy-subrc <> 0.
          RAISE EXCEPTION TYPE z2ui6_cx_util_error EXPORTING val = `LOCK_READ_FAILED`.
        ENDIF.
      ...
      CATCH z2ui6_cx_util_error INTO DATA(lx_error).
        RAISE EXCEPTION lx_error.
      CATCH cx_root INTO DATA(lx_root).
        RAISE EXCEPTION TYPE z2ui6_cx_util_error EXPORTING val = lx_root.
    ENDTRY.
  ...
  METHOD mail_send.
    ...
    IF z2ui6_cl_util=>context_check_abap_cloud( ).
      ...
      TRY.
          ...
          CALL METHOD lo_mail_c->(`SET_SENDER`)
            EXPORTING
              iv_address = z2ui6_cl_util=>context_get_user_tech( ) && `@placeholder.local`.
    ...
    TRY.
        ...
        DATA(lt_lines) = z2ui6_cl_util=>c_split( val = body sep = z2ui6_cl_util=>cv_char_util_newline ).
  ...
  METHOD changdoc_read.
    ...
    IF z2ui6_cl_util=>context_check_abap_cloud( ).
      ...
      TRY.
          ...
          DATA(lt_comp_c) = z2ui6_cl_util=>rtti_get_t_attri_by_table_name( lv_cds ).
...
|x, nsis, objectivec, ocaml, pascal, perl, pgsql, php, plain_text, powershell, praat, prolog, properties, protobuf, python, r, razor, rdoc, rhtml, rst, ruby, rust, sass, scad, scala, scheme, scss, sh, sjs, smarty, snippets, soy_template, space, sql,| &&
  ...
  METHOD source_method_to_file.
    ...
    LOOP AT it_source INTO DATA(lv_source).
      IF strlen( lv_source ) > 1.
        result = result && lv_source+1 && z2ui6_cl_util=>cv_char_util_newline.
      ELSE.
        result = result && z2ui6_cl_util=>cv_char_util_newline.
      ENDIF.
    ENDLOOP.
  ...
  METHOD lock_call_function.
    ...
    TRY.
        ...
        lv_function = z2ui6_cl_util=>c_trim_upper( val ).
        CALL FUNCTION lv_function
          PARAMETER-TABLE lt_param
          EXCEPTION-TABLE lt_exception.
  ...
  METHOD cal_is_workday.
    ...
    IF calendar_id IS NOT INITIAL.
      z2ui6_cl_util=>x_raise( `cal_is_workday: factory calendar support is not yet implemented` ).
    ENDIF.
...
*            )->execute( ).
  ...
  METHOD zip_pack.
    ...
    TRY.
      ...
      CATCH cx_root INTO DATA(x).
        RAISE EXCEPTION TYPE z2ui6_cx_util_error EXPORTING val = x.
    ENDTRY.
