...
CLASS ltcl_unit_test IMPLEMENTATION.
  ...
  METHOD test_raise_with_prev.
    ...
    DATA(lx_prev) = NEW z2ui6_cx_util_error( val = `previous error` ).
  ...
  METHOD test_chain_texts.
    ...
    DATA(lx_inner) = NEW z2ui6_cx_util_error( val = `inner` ).
    DATA(lx_middle) = NEW z2ui6_cx_util_error( val      = `middle`
                                                previous = lx_inner ).
    DATA(lx_outer) = NEW z2ui6_cx_util_error( val      = `outer`
                                               previous = lx_middle ).
