...
CLASS ltcl_unit_test IMPLEMENTATION.
  ...
  METHOD test_empty_range.
    ...
    DATA lt_range TYPE z2ui6_cl_util=>ty_t_range.

    DATA(lo_range) = NEW z2ui6_cl_util_range(
      iv_fieldname = `CARRID`
  ...
  METHOD test_constants.
    ...
    cl_abap_unit_assert=>assert_equals( exp = `I`  act = z2ui6_cl_util_range=>signs-including ).
    cl_abap_unit_assert=>assert_equals( exp = `E`  act = z2ui6_cl_util_range=>signs-excluding ).
    cl_abap_unit_assert=>assert_equals( exp = `EQ` act = z2ui6_cl_util_range=>options-equal ).
    cl_abap_unit_assert=>assert_equals( exp = `NE` act = z2ui6_cl_util_range=>options-not_equal ).
    cl_abap_unit_assert=>assert_equals( exp = `BT` act = z2ui6_cl_util_range=>options-between ).
    cl_abap_unit_assert=>assert_equals( exp = `NB` act = z2ui6_cl_util_range=>options-not_between ).
    cl_abap_unit_assert=>assert_equals( exp = `CP` act = z2ui6_cl_util_range=>options-contains_pattern ).
    cl_abap_unit_assert=>assert_equals( exp = `NP` act = z2ui6_cl_util_range=>options-not_contains_pattern ).
    cl_abap_unit_assert=>assert_equals( exp = `GT` act = z2ui6_cl_util_range=>options-greater_than ).
    cl_abap_unit_assert=>assert_equals( exp = `GE` act = z2ui6_cl_util_range=>options-greater_equal ).
    cl_abap_unit_assert=>assert_equals( exp = `LE` act = z2ui6_cl_util_range=>options-less_equal ).
    cl_abap_unit_assert=>assert_equals( exp = `LT` act = z2ui6_cl_util_range=>options-less_than ).

  ENDMETHOD.

  METHOD test_factory_eq.

    DATA(ls_range) = z2ui6_cl_util_range=>eq( `AA` ).
  ...
  METHOD test_factory_ne.
    ...
    DATA(ls_range) = z2ui6_cl_util_range=>ne( `BB` ).
  ...
  METHOD test_factory_bt.
    ...
    DATA(ls_range) = z2ui6_cl_util_range=>bt( low = `100` high = `500` ).
  ...
  METHOD test_factory_cp.
    ...
    DATA(ls_range) = z2ui6_cl_util_range=>cp( `*test*` ).
  ...
  METHOD test_factory_gt.
    ...
    DATA(ls_range) = z2ui6_cl_util_range=>gt( `100` ).
  ...
  METHOD test_factory_ge.
    ...
    DATA(ls_range) = z2ui6_cl_util_range=>ge( `50` ).
  ...
  METHOD test_factory_lt.
    ...
    DATA(ls_range) = z2ui6_cl_util_range=>lt( `200` ).
  ...
  METHOD test_factory_le.
    ...
    DATA(ls_range) = z2ui6_cl_util_range=>le( `300` ).
  ...
  METHOD test_factory_exclude.
    ...
    DATA(ls_range) = z2ui6_cl_util_range=>eq( val = `XX` sign = `E` ).
  ...
  METHOD test_factory_with_sql.
    ...
    DATA(lt_range) = VALUE z2ui6_cl_util=>ty_t_range(
      ( z2ui6_cl_util_range=>eq( `AA` ) )
      ( z2ui6_cl_util_range=>eq( `BB` ) ) ).
  ...
  METHOD test_get_sql_multi.
    ...
    DATA(lt_r1) = VALUE z2ui6_cl_util=>ty_t_range( ( z2ui6_cl_util_range=>eq( `LH` ) ) ).
    DATA(lt_r2) = VALUE z2ui6_cl_util=>ty_t_range( ( z2ui6_cl_util_range=>bt( low = `100` high = `500` ) ) ).

    DATA(lv_sql1) = NEW z2ui6_cl_util_range( iv_fieldname = `CARRID` ir_range = REF #( lt_r1 ) )->get_sql( ).
    DATA(lv_sql2) = NEW z2ui6_cl_util_range( iv_fieldname = `CONNID` ir_range = REF #( lt_r2 ) )->get_sql( ).

    DATA(lv_result) = z2ui6_cl_util_range=>get_sql_multi( VALUE #( ( lv_sql1 ) ( lv_sql2 ) ) ).
