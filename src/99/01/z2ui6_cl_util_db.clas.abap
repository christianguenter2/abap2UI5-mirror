CLASS z2ui6_cl_util_db DEFINITION
  PUBLIC
  FINAL
  CREATE PUBLIC .
...
CLASS z2ui6_cl_util_db IMPLEMENTATION.



  METHOD delete_by_handle.

    DELETE FROM z2ui6_t_91
        WHERE
           uname = @uname
            AND handle = @handle
            AND handle2 = @handle2
  ...
  METHOD load_multi_by_handle.
    ...
    DATA lr_uname   TYPE RANGE OF z2ui6_t_91-uname.
    DATA lr_handle  TYPE RANGE OF z2ui6_t_91-handle.
    DATA lr_handle2 TYPE RANGE OF z2ui6_t_91-handle2.
    DATA lr_handle3 TYPE RANGE OF z2ui6_t_91-handle3.
  ...
  METHOD save.
    ...
    DATA(ls_db) = VALUE z2ui6_t_91(
        uname   = uname
        handle  = handle
        handle2 = handle2
        handle3 = handle3
        data    = z2ui6_cl_util=>xml_stringify( data ) ).

    IF lv_id IS NOT INITIAL.
      ls_db-id = lv_id.
    ELSE.
      ls_db-id = z2ui6_cl_util=>uuid_get_c32( ).
    ENDIF.

    MODIFY z2ui6_t_91 FROM @ls_db.
    IF sy-subrc <> 0.
      RAISE EXCEPTION TYPE z2ui6_cx_util_error
        EXPORTING
          val = `DB_SAVE_FAILED`.
