CLASS z2ui6_cl_ui5_app_start DEFINITION PUBLIC FINAL.
  ...
  PUBLIC SECTION.
    ...
    CLASS-METHODS factory
      RETURNING
        VALUE(result) TYPE REF TO z2ui6_cl_ui5_app_start.
    ...
    METHODS render_header_toolbar
      IMPORTING page TYPE REF TO z2ui6_cl_ui5_view_builder.
    METHODS render_quickstart
      IMPORTING form TYPE REF TO z2ui6_cl_ui5_view_builder.
    ...
    METHODS create_layout_form
      IMPORTING
        view          TYPE REF TO z2ui6_cl_ui5_view_builder
      RETURNING
        VALUE(result) TYPE REF TO z2ui6_cl_ui5_view_builder.
    METHODS get_app_url
      IMPORTING
  ...
  PRIVATE SECTION.
    ...
    METHODS header_icon
      IMPORTING
        toolbar TYPE REF TO z2ui6_cl_ui5_view_builder
        icon    TYPE string
        tooltip TYPE string
        press   TYPE string.
    ...
    METHODS render_section
      IMPORTING
        form  TYPE REF TO z2ui6_cl_ui5_view_builder
        title TYPE string.
...
CLASS z2ui6_cl_ui5_app_start IMPLEMENTATION.
  ...
  METHOD on_init.
    ...
    reset_button_state( ).
    ms_home-classname = z2ui6_cl_ui5_util_context=>rtti_get_classname_by_ref( NEW z2ui6_cl_ui5_app_hi_world( ) ).

  ENDMETHOD.

  METHOD on_event.

    DATA li_app_config TYPE REF TO z2ui6_if_app.
  ...
  METHOD on_event_check.
    ...
    TRY.
        ms_home-classname = z2ui6_cl_ui5_util_context=>c_trim_upper( ms_home-classname ).
  ...
  METHOD render_start.
    ...
    DATA(view) = z2ui6_cl_ui5_view_builder=>factory( ).
  ...
  METHOD render_samples.
    ...
    IF z2ui6_cl_ui5_util_context=>rtti_check_class_exists( class ).
      lv_class = class.
    ELSEIF class_old IS NOT INITIAL AND z2ui6_cl_ui5_util_context=>rtti_check_class_exists( class_old ).
      lv_class = class_old.
    ENDIF.
  ...
  METHOD render_system_popup.
    ...
    form->tag( `CheckBox`
        )->a( n = `selected`  b = z2ui6_cl_ui5_util_context=>check_abap_cloud( )
        )->a( n = `enabled`   v = `false` ).
    render_text( form  = form
                 label = `User Exit`
                 text  = z2ui6_cl_ui5_user_exit=>get_user_exit_class( ) ).

    render_text( form  = form
                 label = `abap2UI5 Version`
                 text  = z2ui6_if_app=>version ).
  ...
  METHOD get_app_url.
    ...
    result = z2ui6_cl_ui5_handler=>app_get_url( classname = classname
