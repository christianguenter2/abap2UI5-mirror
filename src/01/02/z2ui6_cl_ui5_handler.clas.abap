CLASS z2ui6_cl_ui5_handler DEFINITION PUBLIC FINAL.

  PUBLIC SECTION.
    DATA mo_action       TYPE REF TO z2ui6_cl_ui5_action.
    DATA mv_request_json TYPE string.
    DATA ms_request      TYPE z2ui6_if_ui5_types=>ty_s_request.
    DATA ms_response     TYPE z2ui6_if_ui5_types=>ty_s_response.
    ...
    METHODS main
      RETURNING
        VALUE(result) TYPE z2ui6_if_ui5_types=>ty_s_http_res.

    METHODS request_json_to_abap
      IMPORTING
        val           TYPE string
      RETURNING
        VALUE(result) TYPE z2ui6_if_ui5_types=>ty_s_request.
  ...
  PROTECTED SECTION.
    ...
    METHODS response_abap_to_json
      IMPORTING
        val           TYPE z2ui6_if_ui5_types=>ty_s_response
      RETURNING
        VALUE(result) TYPE string.
  ...
  PRIVATE SECTION.
    ...
    METHODS actions_serialize
      IMPORTING
        ...
        path     TYPE string
        t_action TYPE z2ui6_if_ui5_types=>ty_t_queued_action
      RAISING
    ...
    METHODS request_parse_config
      ...
      CHANGING
        cs_front  TYPE z2ui6_if_ui5_types=>ty_s_request-s_front
      RAISING
        z2ui6_cx_ajson_error.
    ...
    METHODS scroll_pos_read
      IMPORTING
        ...
        iv_path       TYPE string
      RETURNING
        VALUE(result) TYPE z2ui6_if_ui5_types=>ty_s_scroll_pos.
    ...
    METHODS request_app_start
      IMPORTING
        ...
        io_comp_data  TYPE REF TO z2ui6_if_ajson
      RETURNING
        VALUE(result) TYPE string
      RAISING
        z2ui6_cx_ajson_error.
...
CLASS z2ui6_cl_ui5_handler IMPLEMENTATION.
  ...
  METHOD request_parse_event_args.
    ...
    DATA lo_front TYPE REF TO z2ui6_if_ajson.
    ...
    DO.
      ...
      ENDIF.
      CASE lv_node_type.
        WHEN z2ui6_if_ajson_types=>node_type-object OR z2ui6_if_ajson_types=>node_type-array.
  ...
  METHOD request_app_start.
    TRY.
        IF io_comp_data IS BOUND.
          result = z2ui6_cl_ui5_util_context=>c_trim_upper(
    ...
    result = z2ui6_cl_ui5_util_context=>c_trim_upper(
        z2ui6_cl_ui5_util_context=>url_param_get( val = `app_start`
                                                  url = iv_search ) ).
  ...
  METHOD app_get_url.
    ...
    DATA(lt_param) = z2ui6_cl_ui5_util_context=>url_param_get_tab( search ).
    ...
    result = |{ origin }{ pathname }?| && z2ui6_cl_ui5_util_context=>url_param_create_url( lt_param ) && lv_hash.
  ...
  METHOD request_app_start_draft.
    TRY.
        ...
        result = z2ui6_cl_ui5_util_context=>c_trim_upper(
            z2ui6_cl_ui5_util_context=>url_param_get( val = `z2ui5-xapp-state`
                                                      url = lv_hash ) ).
      CATCH cx_root ##NO_HANDLER.
    ENDTRY.
  ENDMETHOD.
  ...
  METHOD request_app_start_route.
    ...
    TRY.
        ...
        lv_rest = cut_at( iv_val = lv_rest
                          iv_sub = `?` ).
        result = z2ui6_cl_ui5_util_context=>c_trim_upper( lv_rest ).
      CATCH cx_root ##NO_HANDLER.
    ENDTRY.
  ENDMETHOD.
  ...
  METHOD response_abap_to_json.
    TRY.
        ...
        li_ajson_result->set( iv_path = `/`
                           iv_val     = ls_front ).
        li_ajson_result = li_ajson_result->filter( z2ui6_cl_ui5_util_json_fl=>create_no_empty_values( ) ).
  ...
  METHOD constructor.
    ...
    mv_request_json = val.
    mo_action = NEW z2ui6_cl_ui5_action( me ).
  ...
  METHOD request_context_info.
    ...
    IF mo_action IS BOUND AND mv_request_parsed = abap_true.
      ...
      IF mo_action->mo_app IS BOUND.
        ...
        IF mo_action->mo_app->mo_app IS BOUND.
          lv_app = z2ui6_cl_ui5_util_context=>rtti_get_classname_by_ref( mo_action->mo_app->mo_app ).
        ENDIF.
      ENDIF.
    ENDIF.
    ...
    DATA(lv_app_start) = z2ui6_cl_ui5_action=>app_start_safe( ms_request-s_control-app_start ).
  ...
  METHOD main_begin.
    ...
    ELSEIF ms_request-s_control-app_start IS NOT INITIAL.
      NEW z2ui6_cl_ui5_srv_draft( )->cleanup( ).
  ...
  METHOD main_end_nav.
    ...
    IF mo_action->ms_next-s_nav-set_nav_routing IS INITIAL
      ...
      mo_action->ms_next-s_nav-set_nav_routing = z2ui6_if_client=>cs_nav_mode-default.
    ENDIF.
  ...
  METHOD main_end.
    ...
    DATA(lo_front) = NEW z2ui6_cl_ui5_frontend( mo_action ).
    ...
    ms_response = VALUE #( s_front-s_action = mo_action->ms_next-s_action
                           ...
                           s_front-app      = z2ui6_cl_ui5_util_context=>rtti_get_classname_by_ref( mo_action->mo_app->mo_app )
                           model            = lv_model ).
  ...
  METHOD main_end_save.
    ...
    ELSEIF mo_action->mo_app->mv_nav_mode = z2ui6_if_client=>cs_nav_mode-keep
  ...
  METHOD main_process.
    ...
    DATA(li_client) = CAST z2ui6_if_client( NEW z2ui6_cl_ui5_client( mo_action ) ).
    ...
    IF mo_action->mo_app->mv_check_sticky = abap_false.
      z2ui6_cl_ui5_util_context=>db_rollback( ).
    ENDIF.
    ...
    IF mo_action->ms_actual-event = z2ui6_if_ui5_types=>cs_event_nav_app_leave.
