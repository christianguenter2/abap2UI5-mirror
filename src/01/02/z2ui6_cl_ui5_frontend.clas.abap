...
CLASS z2ui6_cl_ui5_frontend DEFINITION PUBLIC FINAL CREATE PUBLIC.
  ...
  PRIVATE SECTION.
    ...
    METHODS build_global_call
      IMPORTING
        ...
        opt           TYPE REF TO z2ui6_if_ajson OPTIONAL
      RETURNING
    ...
    METHODS box_resolve
      IMPORTING
        ...
        details       TYPE clike
      RETURNING
        VALUE(result) TYPE z2ui6_cl_ui5_util_context=>ty_s_msg_box.
    ...
    METHODS set_opt_string
      IMPORTING
        json TYPE REF TO z2ui6_if_ajson
        name TYPE string
        val  TYPE clike
      RAISING
...
CLASS z2ui6_cl_ui5_frontend IMPLEMENTATION.
  ...
  METHOD build_global_call.
    ...
    TRY.
        result = z2ui6_cl_ajson=>create_empty( ).
        result->touch_array( `/` ).
        ...
        IF opt IS BOUND AND opt->is_empty( ) = abap_false.
          result->push( iv_path = `/`
                        iv_val  = opt ).
        ENDIF.
      CATCH z2ui6_cx_ajson_error INTO DATA(lx_json).
  ...
  METHOD slot_destroy.
    ...
    INSERT VALUE #( slot   = slot
                    method = z2ui6_if_ui5_types=>cs_slot_action-destroy )
  ...
  METHOD slot_display.
    ...
    TRY.
        ...
        DATA(li_opt) = CAST z2ui6_if_ajson( z2ui6_cl_ajson=>create_empty( ) ).
        set_opt_strings(
            json = li_opt
  ...
  METHOD slots_serialize.
    ...
    IF line_exists( mo_action->ms_next-t_action_front[ slot   = z2ui6_if_client=>cs_view-main
      ...
      DELETE mo_action->ms_next-t_action_front
             WHERE method = z2ui6_if_ui5_types=>cs_slot_action-destroy
               AND ( slot = z2ui6_if_client=>cs_view-popup
                  OR slot = z2ui6_if_client=>cs_view-popover ).
    ENDIF.

    " The view-lifecycle calls leave in SLOT order, never in the order the
    " app happened to make them - see the ABAP Doc.
    LOOP AT VALUE string_table( ( z2ui6_if_client=>cs_view-main )
                                ( z2ui6_if_client=>cs_view-nested )
                                ( z2ui6_if_client=>cs_view-nested2 )
                                ( z2ui6_if_client=>cs_view-popup )
                                ( z2ui6_if_client=>cs_view-popover ) )
         INTO DATA(lv_slot).
      ...
      LOOP AT mo_action->ms_next-t_action_front REFERENCE INTO DATA(lr_action) "#EC CI_SORTSEQ
           WHERE slot = lv_slot.
        DATA(lt_arg) = VALUE string_table( ( z2ui6_if_ui5_types=>cs_slot_action-target )
                                           ( lr_action->method )
                                           ( lr_action->slot ) ).
        IF lr_action->method = z2ui6_if_ui5_types=>cs_slot_action-display.
  ...
  METHOD nav_serialize.
    ...
    TRY.
        ...
        queue_system( t_arg = VALUE #( ( z2ui6_if_ui5_types=>cs_global_target-router )
                                       ( `sync` ) )
                      opt   = li_opt ).
  ...
  METHOD msg_toast.
    ...
    TRY.
        ...
        queue_app( t_arg = VALUE #( ( z2ui6_if_ui5_types=>cs_global_target-message_toast )
                                    ( `show` )
                                    ( CONV string( text ) ) )
                   opt   = li_opt ).
  ...
  METHOD msg_box.
    ...
    TRY.
        ...
        queue_app( t_arg = VALUE #( ( z2ui6_if_ui5_types=>cs_global_target-message_box )
                                    ( ls_msg-type )
                                    ( ls_msg-text ) )
                   opt   = li_opt ).
  ...
  METHOD box_resolve.
    ...
    IF z2ui6_cl_ui5_util_context=>rtti_check_clike( text ) = abap_false.
      result = z2ui6_cl_ui5_util_context=>ui5_msg_box_format( text ).
    ...
    ELSE.
      ...
      result = z2ui6_cl_ui5_util_context=>ui5_data_box_format( text ).
