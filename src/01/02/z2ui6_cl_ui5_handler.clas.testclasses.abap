...
CLASS z2ui6_cl_ui5_handler DEFINITION LOCAL FRIENDS ltcl_00_base ltcl_01_request ltcl_02_response
...
CLASS ltcl_00_base DEFINITION ABSTRACT
  ...
  PROTECTED SECTION.
    ...
    METHODS started_with
      IMPORTING
        io_app        TYPE REF TO z2ui6_if_app
      RETURNING
        VALUE(result) TYPE REF TO z2ui6_cl_ui5_handler.
    ...
    METHODS check_display
      IMPORTING
        io_handler    TYPE REF TO z2ui6_cl_ui5_handler
        iv_slot       TYPE string
      RETURNING
...
CLASS ltcl_00_base IMPLEMENTATION.
  ...
  METHOD started_with.
    ...
    result = NEW z2ui6_cl_ui5_handler( val = `` ).
    result->mo_action->mo_app->mo_app      = io_app.
    result->mo_action->mo_app->ms_draft-id = z2ui6_cl_ui5_util_context=>uuid_get_c32( ).
    result->main_loop( ).
  ...
  METHOD check_display.
    ...
    result = xsdbool( line_exists( io_handler->mo_action->ms_next-t_action_front[
                                       slot   = iv_slot
                                       method = z2ui6_if_ui5_types=>cs_slot_action-display ] ) ). "#EC CI_SORTSEQ
...
CLASS ltcl_01_request IMPLEMENTATION.
  ...
  METHOD test_constructor.
    ...
    DATA lo_handler TYPE REF TO z2ui6_cl_ui5_handler.
  ...
  METHOD load_startup_app.
    ...
    DATA lo_post TYPE REF TO z2ui6_cl_ui5_handler.
    DATA lo_startup TYPE REF TO z2ui6_cl_ui5_app_start.
  ...
  METHOD test_request_parse.
    ...
    DATA lv_payload TYPE string.
    DATA lo_handler TYPE REF TO z2ui6_cl_ui5_handler.
    DATA ls_request TYPE z2ui6_if_ui5_types=>ty_s_request.
  ...
  METHOD test_context_info_stale_action.
    ...
    lo_handler = NEW #( val = `` ).
    lo_handler->mo_action = NEW z2ui6_cl_ui5_action( lo_handler ).
  ...
  METHOD test_app_start_encoded_slash.
    ...
    DATA lo_handler TYPE REF TO z2ui6_cl_ui5_handler.
    DATA lo_no_comp_data TYPE REF TO z2ui6_if_ajson.
    lo_handler = NEW #( val = `` ).
  ...
  METHOD test_hash_app_part.
    ...
    cl_abap_unit_assert=>assert_equals( exp = `/app/ZCL_X/D1`
                                        act = z2ui6_cl_ui5_handler=>hash_get_app_part( `#/app/ZCL_X/D1` ) ).

    " launchpad - the shell owns everything before '&/'
    cl_abap_unit_assert=>assert_equals( exp = `app/ZCL_X/D1`
                                        act = z2ui6_cl_ui5_handler=>hash_get_app_part( `#Z2UI5-display&/app/ZCL_X/D1` ) ).

    " ... including a shell hash that carries its own parameters
    cl_abap_unit_assert=>assert_equals( exp = `app/ZCL_X`
                                        act = z2ui6_cl_ui5_handler=>hash_get_app_part( `#Z2UI5-display?a=b&c=d&/app/ZCL_X` ) ).

    " an app hash containing '&/' must not be split
    cl_abap_unit_assert=>assert_equals( exp = `/app/ZCL_X/D1?next=&/y`
                                        act = z2ui6_cl_ui5_handler=>hash_get_app_part( `#/app/ZCL_X/D1?next=&/y` ) ).

    cl_abap_unit_assert=>assert_equals( exp = ``
                                        act = z2ui6_cl_ui5_handler=>hash_get_app_part( `` ) ).
  ...
  METHOD test_hash_shell_part.
    ...
    cl_abap_unit_assert=>assert_equals(
        exp = `Z2UI5-display`
        act = z2ui6_cl_ui5_handler=>hash_get_shell_part( `#Z2UI5-display&/app/ZCL_X/D1` ) ).

    " ... including a shell hash that carries its own parameters
    cl_abap_unit_assert=>assert_equals(
        exp = `Z2UI5-display?a=b&c=d`
        act = z2ui6_cl_ui5_handler=>hash_get_shell_part( `#Z2UI5-display?a=b&c=d&/app/ZCL_X` ) ).
    ...
    cl_abap_unit_assert=>assert_equals(
        exp = ``
        act = z2ui6_cl_ui5_handler=>hash_get_shell_part( `#/app/ZCL_X/D1` ) ).
    cl_abap_unit_assert=>assert_equals(
        exp = ``
        act = z2ui6_cl_ui5_handler=>hash_get_shell_part( `#/app/ZCL_X/D1?next=&/y` ) ).

    " a bare shell hash without an app part keeps nothing - same contract as
    " Router.splitHash, whose shell is only ever cut at '&/'
    cl_abap_unit_assert=>assert_equals(
        exp = ``
        act = z2ui6_cl_ui5_handler=>hash_get_shell_part( `#Z2UI5-display` ) ).

    cl_abap_unit_assert=>assert_equals(
        exp = ``
        act = z2ui6_cl_ui5_handler=>hash_get_shell_part( `` ) ).
  ...
  METHOD test_app_get_url.
    ...
    cl_abap_unit_assert=>assert_equals(
        ...
        act = z2ui6_cl_ui5_handler=>app_get_url(
                  classname = `ZCL_NEW`
                  origin    = `https://h`
                  pathname  = `/p`
                  search    = ``
...
CLASS ltcl_02_response IMPLEMENTATION.
  ...
  METHOD test_response_json.
    ...
    DATA lo_handler TYPE REF TO z2ui6_cl_ui5_handler.
    DATA ls_response TYPE z2ui6_if_ui5_types=>ty_s_response.
  ...
  METHOD test_response_actions_embedded.
    ...
    lo_handler = NEW #( val = `` ).
    ls_response-s_front-id = `ID123`.
    INSERT VALUE #( o_json = z2ui6_cl_ajson=>parse( `["CONTROL_BY_ID","tab","","setHiddenInPopin",{"A":1}]` ) )
           INTO TABLE ls_response-s_front-s_action-t_system.
    INSERT VALUE #( o_json = z2ui6_cl_ajson=>parse( `["SET_FOCUS","id1"]` ) )
  ...
  METHOD test_view_update_popup.
    ...
    cl_abap_unit_assert=>assert_true( check_display( io_handler = lo_handler
                                                     iv_slot    = z2ui6_if_client=>cs_view-popup ) ).
  ...
  METHOD test_auto_update_push.
    ...
    lo_handler->mo_action->mo_app->ms_draft-id = z2ui6_cl_ui5_util_context=>uuid_get_c32( ).
  ...
  METHOD test_model_client_stored.
    ...
    DATA(lo_model) = NEW z2ui6_cl_ui5_srv_model( attri = lo_handler->mo_action->mo_app->mt_attri
                                                  app  = lo_app ).
  ...
  METHOD test_delta_drops_client.
    ...
    lo_handler->mo_action->mo_app->mo_app = NEW ltcl_app_noop( ).
    lo_handler->mo_action->mo_app->mv_model_client = `{"SENTINEL":true}`.
    lo_handler->ms_request-o_model = z2ui6_cl_ajson=>parse( `{"NAME":"changed"}` ).
  ...
  METHOD test_system_slot_order.
    ...
    NEW z2ui6_cl_ui5_frontend( lo_handler->mo_action )->slots_serialize( ).
...
CLASS ltcl_03_dispatch IMPLEMENTATION.
  ...
  METHOD test_sticky_keep_saves_draft.
    ...
    DATA lo_handler TYPE REF TO z2ui6_cl_ui5_handler.
    DATA(lo_draft) = NEW z2ui6_cl_ui5_srv_draft( ).
    ...
    lo_handler->mo_action->mo_app->mv_nav_mode     = z2ui6_if_client=>cs_nav_mode-keep.
