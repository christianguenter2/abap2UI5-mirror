CLASS z2ui6_cl_ui5_srv_event DEFINITION PUBLIC FINAL.
  ...
  PUBLIC SECTION.
    METHODS get_event
      IMPORTING
        ...
        s_cnt         TYPE z2ui6_if_client=>ty_s_event_control OPTIONAL
    ...
    METHODS get_event_client
      IMPORTING
        val           TYPE clike
        view          TYPE clike        DEFAULT z2ui6_if_client=>cs_view-main
        t_arg         TYPE string_table OPTIONAL
      RETURNING
        VALUE(result) TYPE string.
...
CLASS z2ui6_cl_ui5_srv_event IMPLEMENTATION.
  ...
  METHOD get_event_client.
    ...
    result = |{ z2ui6_if_ui5_types=>cs_ui5-event_frontend_function }('{ escape_js_string( ls_event-val ) }'{ get_t_arg( ls_event-t_arg ) }|.
  ...
  METHOD map_client_event.
    ...
    DATA(lv_slot) = SWITCH string( lv_val
                                   WHEN z2ui6_if_client=>cs_event-nav_container_to         THEN z2ui6_if_client=>cs_view-main
                                   WHEN z2ui6_if_client=>cs_event-nest_nav_container_to    THEN z2ui6_if_client=>cs_view-nested
                                   WHEN z2ui6_if_client=>cs_event-nest2_nav_container_to   THEN z2ui6_if_client=>cs_view-nested2
                                   WHEN z2ui6_if_client=>cs_event-popup_nav_container_to   THEN z2ui6_if_client=>cs_view-popup
                                   WHEN z2ui6_if_client=>cs_event-popover_nav_container_to THEN z2ui6_if_client=>cs_view-popover
                                   ELSE `` ).
    IF lv_slot IS NOT INITIAL.
      ...
      lv_val = z2ui6_if_client=>cs_event-control_by_id.
    ELSEIF lv_val = z2ui6_if_client=>cs_event-popup_close
        OR lv_val = z2ui6_if_client=>cs_event-popover_close.
      ...
      lt_arg = VALUE #( ( z2ui6_if_ui5_types=>cs_slot_action-target )
                        ( z2ui6_if_ui5_types=>cs_slot_action-destroy )
                        ( COND #( WHEN lv_val = z2ui6_if_client=>cs_event-popup_close
                                  THEN z2ui6_if_client=>cs_view-popup
                                  ELSE z2ui6_if_client=>cs_view-popover ) ) ).
      lv_val = z2ui6_if_client=>cs_event-control_global.
    ELSEIF lv_val = z2ui6_if_client=>cs_event-control_by_id.
      ...
      DATA(lv_view_slot) = COND string( WHEN view = z2ui6_if_client=>cs_view-main THEN ``
                                        ELSE CONV string( view ) ).
      INSERT lv_view_slot INTO lt_arg INDEX 2.
    ELSEIF lv_val = z2ui6_if_client=>cs_event-bind_element.
  ...
  METHOD get_event_client_json.
    ...
    TRY.
      ...
      CATCH z2ui6_cx_ajson_error INTO DATA(lx_error).
  ...
  METHOD get_event_client_ajson.
    ...
    TRY.
        DATA(li_json) = CAST z2ui6_if_ajson( z2ui6_cl_ajson=>create_empty( ) ).
        li_json->touch_array( `/` ).
        ...
        LOOP AT ls_event-t_arg INTO DATA(lv_arg).
          ...
          IF lv_arg IS NOT INITIAL AND ( lv_arg(1) = `{` OR lv_arg(1) = `[` ).
            ...
            TRY.
                li_json->push( iv_path = `/`
                               iv_val  = z2ui6_cl_ajson=>parse( lv_arg ) ).
  ...
  METHOD escape_js_string.
    ...
    result = val.
    IF result NA `\'` AND result NA z2ui6_cl_ui5_util_context=>cv_char_util_cr_lf.
      RETURN.
    ENDIF.
    ...
    REPLACE ALL OCCURRENCES OF z2ui6_cl_ui5_util_context=>cv_char_util_cr_lf IN result WITH `\n`.
    REPLACE ALL OCCURRENCES OF z2ui6_cl_ui5_util_context=>cv_char_util_newline IN result WITH `\n`.
    " a standalone CR (not part of CR+LF, already collapsed above) is a JS
    " line terminator too and would break the '...' literal
    REPLACE ALL OCCURRENCES OF z2ui6_cl_ui5_util_context=>cv_char_util_cr_lf(1)
            IN result WITH `\r`.
