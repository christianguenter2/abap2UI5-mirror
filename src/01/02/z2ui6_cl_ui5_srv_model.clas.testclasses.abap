...
CLASS ltcl_app_tree IMPLEMENTATION.
  METHOD z2ui6_if_app~main ##NEEDED.
  ENDMETHOD.
ENDCLASS.
...
CLASS ltcl_app_shapes DEFINITION FINAL
  ...
  PUBLIC SECTION.
    ...
    DATA mi_app  TYPE REF TO z2ui6_if_app.
...
CLASS ltcl_shp_filter IMPLEMENTATION.
  ...
  METHOD z2ui6_if_ajson_filter~keep_node.
    ...
    rv_keep = xsdbool( is_node-type <> z2ui6_if_ajson_types=>node_type-string OR is_node-value IS NOT INITIAL ).
  ENDMETHOD.
...
CLASS z2ui6_cl_ui5_srv_model DEFINITION LOCAL FRIENDS ltcl_00_base ltcl_01_dissolve ltcl_02_search
...
CLASS ltcl_00_base DEFINITION ABSTRACT
  ...
  PROTECTED SECTION.
    ...
    DATA mo_model TYPE REF TO z2ui6_cl_ui5_srv_model.
    DATA mr_attri TYPE REF TO z2ui6_if_ui5_types=>ty_t_attri.
...
CLASS ltcl_00_base IMPLEMENTATION.
  ...
  METHOD roundtrip.
    ...
    DATA(lv_app_xml)   = z2ui6_cl_ui5_util_context=>xml_stringify( mo_app ).
    DATA(lv_attri_xml) = z2ui6_cl_ui5_util_context=>xml_stringify( mr_attri->* ).
    ...
    CLEAR mo_app.
    z2ui6_cl_ui5_util_context=>xml_parse( EXPORTING xml = lv_app_xml
                                          IMPORTING any = mo_app ).
    CREATE DATA mr_attri.
    z2ui6_cl_ui5_util_context=>xml_parse( EXPORTING xml = lv_attri_xml
  ...
  METHOD inv_types_known.
    ...
    LOOP AT mt_bound INTO DATA(lv_name).
      ...
      IF ls_parent-type_kind <> z2ui6_cl_ui5_util_context=>cv_typedescr_typekind_dref.
        CONTINUE.
      ENDIF.
...
CLASS ltcl_01_dissolve IMPLEMENTATION.
  ...
  METHOD deep_struct_every_level.
    ...
    DATA(lo_model) = NEW z2ui6_cl_ui5_srv_model( attri = REF #( lt_attri )
                                                 app   = lo_app ).
  ...
  METHOD refresh_keeps_bindings.
    ...
    lr_attri->custom_mapper = z2ui6_cl_ajson_mapping=>create_upper_case( ).
...
CLASS ltcl_02_search IMPLEMENTATION.
  ...
  METHOD reference_itself_refused.
    ...
    TRY.
      ...
      CATCH z2ui6_cx_ui5_util_error INTO DATA(lx).
  ...
  METHOD unreachable_row_skipped.
    ...
    DATA(lo_descr) = z2ui6_cl_ui5_util_context=>rtti_get_typedescr_by_data_ref( REF #( mo_app->mv_string ) ).
  ...
  METHOD recreated_value_found.
    ...
    TRY.
      ...
      CATCH z2ui6_cx_ui5_util_error INTO lx.
        cl_abap_unit_assert=>assert_true( xsdbool( lx->get_text( ) CS `BINDING_ERROR` ) ).
    ENDTRY.
...
CLASS ltcl_03_model_out IMPLEMENTATION.
  ...
  METHOD values_per_form.
    ...
    bind_all( ).
    DATA(lo_json) = CAST z2ui6_if_ajson( z2ui6_cl_ajson=>parse( mo_model->main_json_stringify( ) ) ).
  ...
  METHOD markup_escaped.
    ...
    DATA(lo_json) = CAST z2ui6_if_ajson( z2ui6_cl_ajson=>parse( lv_json ) ).
  ...
  METHOD json_bind_spliced.
    ...
    DATA(lo_result) = CAST z2ui6_if_ajson( z2ui6_cl_ajson=>parse( mo_model->main_json_stringify( ) ) ).
    cl_abap_unit_assert=>assert_equals( exp = z2ui6_if_ajson_types=>node_type-object
  ...
  METHOD mapper_applied.
    ...
    DATA(lr_attri) = bind( REF #( mo_app->ms_flat ) ).
    lr_attri->custom_mapper = z2ui6_cl_ajson_mapping=>create_lower_case( ).
...
CLASS ltcl_04_model_in DEFINITION INHERITING FROM ltcl_00_base FINAL
  ...
  PRIVATE SECTION.
    ...
    METHODS typed_model
      ...
      RETURNING
        VALUE(result) TYPE REF TO z2ui6_cl_ui5_srv_model.
    METHODS tree_app
      RETURNING
    ...
    METHODS delta
      IMPORTING
        iv_json       TYPE string
      RETURNING
        VALUE(result) TYPE REF TO z2ui6_if_ajson
      RAISING
...
CLASS ltcl_04_model_in IMPLEMENTATION.
  ...
  METHOD typed_model.
    ...
    DATA lr_attri TYPE REF TO z2ui6_if_ui5_types=>ty_t_attri.
  ...
  METHOD delta.
    ...
    result = z2ui6_cl_ajson=>parse( iv_json ).
  ...
  METHOD whole_value_per_form.
    ...
    bind_all( ).
    DATA(lo_front) = CAST z2ui6_if_ajson( z2ui6_cl_ajson=>create_empty( ) ).
  ...
  METHOD whole_scalar_typed.
    ...
    lo_front = CAST z2ui6_if_ajson( z2ui6_cl_ajson=>create_empty( ) ).
    lo_front->set( iv_path = `/MV_DATE`
                   iv_val  = `20240116` ).
  ...
  METHOD markup_round_trips.
    ...
    mo_model->main_json_to_attri( CAST z2ui6_if_ajson( z2ui6_cl_ajson=>parse( lv_out ) ) ).
  ...
  METHOD whole_table_round_trips.
    ...
    mo_model->main_json_to_attri( CAST z2ui6_if_ajson( z2ui6_cl_ajson=>parse( lv_json ) ) ).
  ...
  METHOD delta_nested.
    ...
    DATA lr_attri TYPE REF TO z2ui6_if_ui5_types=>ty_t_attri.
    CREATE DATA lr_attri.
    DATA(lo_model) = NEW z2ui6_cl_ui5_srv_model( attri = lr_attri
                                                 app   = lo_app ).
...
CLASS ltcl_05_draft IMPLEMENTATION.
  ...
  METHOD payload_one_document_old.
    ...
    DATA(lr_val) = z2ui6_cl_ui5_util_context=>xml_srtti_parse_pair( iv_type = lr_row->srtti_type
                                                                    iv_data = lr_row->srtti_data ).
    ASSIGN lr_val->* TO <val>.
    lr_row->srtti_data = z2ui6_cl_ui5_util_context=>xml_srtti_stringify( <val> ).
  ...
  METHOD payload_on_canonical_row.
    ...
    DATA(lv_app_xml)   = z2ui6_cl_ui5_util_context=>xml_stringify( lo_app ).
    DATA(lv_attri_xml) = z2ui6_cl_ui5_util_context=>xml_stringify( lr_attri->* ).
    CLEAR lo_app.
    z2ui6_cl_ui5_util_context=>xml_parse( EXPORTING xml = lv_app_xml
  ...
  METHOD attribute_gone_skipped.
    ...
    INSERT VALUE #( name            = `MR_GONE`
                    check_dissolved = abap_true
                    type_kind       = z2ui6_cl_ui5_util_context=>cv_typedescr_typekind_dref
                    srtti_data      = `payload of a reference nobody has` ) INTO TABLE mr_attri->*.
    INSERT VALUE #( name            = `MR_GONE->*`
                    name_parent     = `MR_GONE`
                    check_dissolved = abap_true
                    type_kind       = z2ui6_cl_ui5_util_context=>cv_typedescr_typekind_table
  ...
  METHOD class_swap_before_load.
    ...
    lo_model->main_attri_db_save_srtti( ).
    DATA(lv_app_xml)   = z2ui6_cl_ui5_util_context=>xml_stringify( lo_host ).
    DATA(lv_attri_xml) = z2ui6_cl_ui5_util_context=>xml_stringify( lr_attri->* ).
    CLEAR lo_host.
    z2ui6_cl_ui5_util_context=>xml_parse( EXPORTING xml = lv_app_xml
  ...
  METHOD class_swap_after_load.
    ...
    lo_model->main_attri_db_save_srtti( ).
    lv_app_xml   = z2ui6_cl_ui5_util_context=>xml_stringify( lo_host ).
    lv_attri_xml = z2ui6_cl_ui5_util_context=>xml_stringify( lr_attri->* ).
    CLEAR lo_host.
    z2ui6_cl_ui5_util_context=>xml_parse( EXPORTING xml = lv_app_xml
                                          IMPORTING any = lo_host ).
    CREATE DATA lr_attri.
  ...
  METHOD bind_options_survive.
    ...
    lr_flat->custom_mapper = z2ui6_cl_ajson_mapping=>create_lower_case( ).
    mo_app->mv_string = `{"sap.app":{"type":"card"}}`.
    DATA(lr_json) = bind( REF #( mo_app->mv_string ) ).
    lr_json->check_json = abap_true.
    DATA(lv_before) = mo_model->main_json_stringify( ).
  ...
  METHOD cell_bind_after_restore.
    ...
    DATA(lo_cont) = NEW z2ui6_cl_ui5_app_cont( ).
    lo_cont->mo_app   = mo_app.
    lo_cont->mt_attri = mr_attri.
    DATA(lo_bind) = NEW z2ui6_cl_ui5_srv_bind( lo_cont ).
  ...
  METHOD alias_repointed_survives.
    ...
    DATA(lo_tab) = CAST cl_abap_tabledescr( z2ui6_cl_ui5_util_context=>rtti_get_typedescr_by_data_ref( mo_app->mr_shared_b ) ).
  ...
  METHOD live_recreated_same_type.
    ...
    DATA(lo_tab) = CAST cl_abap_tabledescr( z2ui6_cl_ui5_util_context=>rtti_get_typedescr_by_data_ref( mo_app->mr_shared_a ) ).
  ...
  METHOD live_recreated_other_type.
    ...
    cl_abap_unit_assert=>assert_equals( exp = 7
                                        act = <elem> ).
    cl_abap_unit_assert=>assert_equals( exp = cl_abap_typedescr=>typekind_int
                                        act = z2ui6_cl_ui5_util_context=>rtti_get_type_kind( <elem> ) ).
