...
CLASS ltcl_test_client DEFINITION FINAL
  ...
  PRIVATE SECTION.
    DATA mo_client TYPE REF TO z2ui6_cl_ui5_client.
...
CLASS z2ui6_cl_ui5_client DEFINITION LOCAL FRIENDS ltcl_test_client.
...
CLASS ltcl_test_client IMPLEMENTATION.
  ...
  METHOD test_view_display.
    ...
    DATA li_client TYPE REF TO z2ui6_if_client.
  ...
  METHOD test_set_nav_routing.
    ...
    li_client->follow_up_action( val   = z2ui6_if_client=>cs_event-set_nav_routing
                                 t_arg = VALUE #( ( z2ui6_if_client=>cs_nav_mode-fresh ) ) ).

    cl_abap_unit_assert=>assert_equals( exp = z2ui6_if_client=>cs_nav_mode-fresh
  ...
  METHOD test_set_nav_routing_default.
    ...
    li_client->follow_up_action( z2ui6_if_client=>cs_event-set_nav_routing ).

    cl_abap_unit_assert=>assert_equals( exp = z2ui6_if_client=>cs_nav_mode-keep
                                        act = mo_action->mo_app->mv_nav_mode ).
  ...
  METHOD test_hash_attach_changed.
    ...
    li_client->follow_up_action( val   = z2ui6_if_client=>cs_event-hash_attach_changed
    ...
    li_client->follow_up_action( z2ui6_if_client=>cs_event-hash_attach_changed ).
  ...
  METHOD test_hash_replace.
    ...
    li_client->follow_up_action( val   = z2ui6_if_client=>cs_event-hash_replace
                                 t_arg = VALUE #( ( `/other` ) ) ).
  ...
  METHOD test_follow_up_action_ev.
    ...
    li_client->follow_up_action( val   = z2ui6_if_client=>cs_event-set_title
                                 t_arg = VALUE #( ( `My Title` ) ) ).
    li_client->follow_up_action( z2ui6_if_client=>cs_event-location_reload ).
  ...
  METHOD test_follow_up_action_nav.
    ...
    li_client->follow_up_action( val   = z2ui6_if_client=>cs_event-nav_container_to
                                 t_arg = VALUE #( ( `myContainer` ) ( `myPage` ) ) ).
    li_client->follow_up_action( val   = z2ui6_if_client=>cs_event-popup_nav_container_to
  ...
  METHOD test_follow_up_action_ctrl.
    ...
    li_client->follow_up_action( val   = z2ui6_if_client=>cs_event-control_global
                                 t_arg = VALUE #( ( `MESSAGE_TOAST` ) ( `show` ) ( `Hello` ) ) ).
    li_client->follow_up_action( val   = z2ui6_if_client=>cs_event-control_by_id
  ...
  METHOD test_check_on_init.
    ...
    cl_abap_unit_assert=>assert_equals( exp = abap_true
                                        act = mo_client->z2ui6_if_client~check_on_init( ) ).
  ...
  METHOD test_nav_app_call_id_stable.
    ...
    cl_abap_unit_assert=>assert_equals( exp = lv_id_first
                                        act = lv_id_second ).
    cl_abap_unit_assert=>assert_equals( exp = lv_id_first
                                        act = lo_new_app->z2ui6_if_app~id_app ).
  ...
  METHOD test_get_comp_params_memo.
    ...
    mo_action->mo_handler->ms_request-s_front-o_comp_data =
        z2ui6_cl_ajson=>parse( `{"startupParameters":{"foo":["bar"],"qty":["7","8"]}}` ).
  ...
  METHOD test_omit_initial_paths.
    ...
    DATA(li_filter) = CAST z2ui6_if_ajson_filter(
    ...
    cl_abap_unit_assert=>assert_equals(
        ...
        act = li_filter->keep_node( is_node  = VALUE #( name = `MIN` type = `object` )
                                    iv_visit = z2ui6_if_ajson_filter=>visit_type-open ) ).
  ...
  METHOD test_omit_initial_decimals.
    ...
    DATA(lo_ajson) = CAST z2ui6_if_ajson( z2ui6_cl_ajson=>create_empty( ) ).
    ...
    ls_row-price = '0.01'.
    lo_ajson = z2ui6_cl_ajson=>create_empty( ).
    lo_ajson->set( iv_ignore_empty = abap_false
                   iv_path         = `/row`
                   iv_val          = ls_row ).
  ...
  METHOD test_omit_filters_serial.
    ...
    DATA li_paths TYPE REF TO z2ui6_if_ajson_filter.

    li_omit = NEW lcl_empty_filter_keep_rows( ).
    cl_abap_unit_assert=>assert_true(
        z2ui6_cl_ui5_util_context=>rtti_check_serializable( li_omit ) ).

    li_paths = NEW lcl_initial_paths_filter( VALUE #( ( `MIN` ) ) ).
    cl_abap_unit_assert=>assert_true(
        z2ui6_cl_ui5_util_context=>rtti_check_serializable( li_paths ) ).

    cl_abap_unit_assert=>assert_true(
        z2ui6_cl_ui5_util_context=>rtti_check_serializable(
    ...
    cl_abap_unit_assert=>assert_false(
        z2ui6_cl_ui5_util_context=>rtti_check_serializable( NEW ltcl_bad_filter( ) ) ).
  ...
  METHOD test_omit_initial_db_save.
    ...
    DATA li_client TYPE REF TO z2ui6_if_client.
    DATA lo_app TYPE REF TO ltcl_test_app.
    DATA lo_cont TYPE REF TO z2ui6_cl_ui5_app_cont.
    DATA lo_cont_db TYPE REF TO z2ui6_cl_ui5_app_cont.
    ...
    lo_cont->db_save( ).
    z2ui6_cl_ui5_app_cont=>db_load_buffer_clear( ).

    lo_cont_db = z2ui6_cl_ui5_app_cont=>db_load( `TEST_OMIT_INITIAL_DRAFT` ).
    ...
    DATA lr_attri TYPE REF TO z2ui6_if_ui5_types=>ty_s_attri.
...
CLASS ltcl_test_model_skipped DEFINITION FINAL
  ...
  PRIVATE SECTION.
    ...
    METHODS setup RAISING z2ui6_cx_ajson_error.
...
CLASS ltcl_test_model_skipped IMPLEMENTATION.
  ...
  METHOD setup.
    ...
    DATA(lo_http) = NEW z2ui6_cl_ui5_handler( val = `` ).
    mo_action = NEW #( val = lo_http ).
    mo_app = NEW #( ).
  ...
  METHOD roundtrip.
    ...
    IF model IS NOT INITIAL.
      mo_action->ms_actual-t_model_skipped = mo_action->mo_app->model_json_parse( z2ui6_cl_ajson=>parse( model ) ).
    ENDIF.
    mo_action->ms_actual-event = event.

    DATA(lo_client) = NEW z2ui6_cl_ui5_client( action = mo_action ).
    mo_app->z2ui6_if_app~main( lo_client ).
