...
CLASS z2ui6_cl_ui5_app_cont DEFINITION LOCAL FRIENDS ltcl_00_base ltcl_01_xml ltcl_02_db ltcl_03_errors ltcl_04_model.
...
CLASS ltcl_00_base DEFINITION ABSTRACT
  ...
  PROTECTED SECTION.
    ...
    DATA mo_cont TYPE REF TO z2ui6_cl_ui5_app_cont.
    ...
    METHODS bind
      IMPORTING
        ...
        iv_path       TYPE string
        io_cont       TYPE REF TO z2ui6_cl_ui5_app_cont OPTIONAL
      RETURNING
    ...
    METHODS load_fresh
      IMPORTING
        iv_id         TYPE string
      RETURNING
        VALUE(result) TYPE REF TO z2ui6_cl_ui5_app_cont.

    METHODS app_of
      IMPORTING
        io_cont       TYPE REF TO z2ui6_cl_ui5_app_cont
      RETURNING
...
CLASS ltcl_00_base IMPLEMENTATION.
  ...
  METHOD setup.
    ...
    mo_cont->ms_draft-id = z2ui6_cl_ui5_util_context=>uuid_get_c32( ).
  ...
  METHOD load_fresh.
    ...
    z2ui6_cl_ui5_app_cont=>db_load_buffer_clear( ).
    result = z2ui6_cl_ui5_app_cont=>db_load( iv_id ).
...
CLASS ltcl_01_xml IMPLEMENTATION.
  ...
  METHOD parse_gives_new_container.
    ...
    DATA(lo_parsed) = z2ui6_cl_ui5_app_cont=>all_xml_parse( mo_cont->all_xml_stringify( ) ).
...
CLASS ltcl_02_db IMPLEMENTATION.
  ...
  METHOD save_marks_app.
    ...
    cl_abap_unit_assert=>assert_equals( exp = mo_cont->ms_draft-id
                                        act = mo_user->z2ui6_if_app~id_draft ).
    DATA(lo_loaded) = load_fresh( mo_cont->ms_draft-id ).
    cl_abap_unit_assert=>assert_equals( exp = abap_true
                                        act = lo_loaded->mv_check_initialized ).
    DATA(lo_app) = app_of( lo_loaded ).
    cl_abap_unit_assert=>assert_equals( exp = mo_cont->ms_draft-id
                                        act = lo_app->z2ui6_if_app~id_draft ).
  ...
  METHOD load_buffered_per_request.
    ...
    DATA(lo_second) = z2ui6_cl_ui5_app_cont=>db_load( mo_cont->ms_draft-id ).
    cl_abap_unit_assert=>assert_true( act = xsdbool( lo_first = lo_second )
                                      msg = `two containers for one draft in one request` ).

    z2ui6_cl_ui5_app_cont=>db_load_buffer_clear( ).
    cl_abap_unit_assert=>assert_initial( z2ui6_cl_ui5_app_cont=>mt_buffer ).
    DATA(lo_third) = z2ui6_cl_ui5_app_cont=>db_load( mo_cont->ms_draft-id ).
  ...
  METHOD second_roundtrip_clean.
    ...
    lo_loaded->ms_draft-id = z2ui6_cl_ui5_util_context=>uuid_get_c32( ).
    lo_loaded->db_save( ).
    check_restored( lo_app ).
  ...
  METHOD load_by_app_same_instance.
    ...
    DATA(lo_loaded) = z2ui6_cl_ui5_app_cont=>db_load( mo_cont->ms_draft-id ).
    DATA(lo_by_app) = z2ui6_cl_ui5_app_cont=>db_load_by_app( app_of( lo_loaded ) ).
    ...
    DATA(lo_other) = z2ui6_cl_ui5_app_cont=>db_load_by_app( mo_user ).
  ...
  METHOD load_by_app_live_instance.
    ...
    z2ui6_cl_ui5_app_cont=>db_load_buffer_clear( ).
    DATA(lo_loaded) = z2ui6_cl_ui5_app_cont=>db_load_by_app( mo_user ).
  ...
  METHOD load_by_app_fills_buffer.
    ...
    z2ui6_cl_ui5_app_cont=>db_load_buffer_clear( ).
    DATA(lo_by_app) = z2ui6_cl_ui5_app_cont=>db_load_by_app( mo_user ).
    DATA(lo_by_id)  = z2ui6_cl_ui5_app_cont=>db_load( mo_cont->ms_draft-id ).
...
CLASS ltcl_03_errors IMPLEMENTATION.
  ...
  METHOD save_with_broken_payloads.
    ...
    DATA(lv_xml) = z2ui6_cl_ui5_util_context=>xml_stringify( mo_cont ).
    NEW z2ui6_cl_ui5_srv_draft( )->create( draft     = mo_cont->ms_draft
                                           model_xml = lv_xml ).
  ...
  METHOD broken_payload_bound_loud.
    ...
    z2ui6_cl_ui5_app_cont=>db_load_buffer_clear( ).
    TRY.
        z2ui6_cl_ui5_app_cont=>db_load( mo_cont->ms_draft-id ).
  ...
  METHOD unknown_draft_raises.
    ...
    z2ui6_cl_ui5_app_cont=>db_load_buffer_clear( ).
    TRY.
        z2ui6_cl_ui5_app_cont=>db_load( `NO_SUCH_DRAFT_` && z2ui6_cl_ui5_util_context=>uuid_get_c32( ) ).
...
CLASS ltcl_04_model IMPLEMENTATION.
  ...
  METHOD stringify_bound_rows_only.
    ...
    DATA(lo_json) = z2ui6_cl_ajson=>parse( mo_cont->model_json_stringify( ) ).
  ...
  METHOD parse_writes_back.
    ...
    DATA(lo_front) = z2ui6_cl_ajson=>parse(
