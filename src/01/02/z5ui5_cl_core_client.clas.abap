...
*    mo_action->ms_next-s_set-s_follow_up_action-custom_js = val.
  ...
  METHOD z5ui5_if_client~get_event_arg.
  ...
  METHOD z5ui5_if_client~message_box_display.
    ...
    IF z5ui5_cl_util=>rtti_check_clike( text ) = abap_false.
      DATA(lt_msg) = z5ui5_cl_util=>msg_get( text ).
      IF lines( lt_msg ) = 1.
        DATA(lv_text) = lt_msg[ 1 ]-text.

        DATA(lv_type) = z5ui5_cl_util=>ui5_get_msg_type( lt_msg[ 1 ]-type ).
      ...
      ELSEIF lines( lt_msg ) > 1.
        ...
        IF title IS INITIAL.
          lv_title = SWITCH #( lt_msg[ 1 ]-type
                               ...
                               ELSE          `Information` ).
        ENDIF.
        lv_type = z5ui5_cl_util=>ui5_get_msg_type( lt_msg[ 1 ]-type ).
      ELSE.
        RETURN.
      ENDIF.
    ELSE.
  ...
  METHOD z5ui5_if_client~nav_app_call.
    ...
    IF app IS NOT BOUND.
      z5ui5_cl_util=>x_raise( `NAV_APP_LEAVE_TO_INITIAL_APP_ERROR` ).
    ENDIF.
  ...
  METHOD z5ui5_if_client~_bind.
    ...
    DATA(lo_bind) = NEW z5ui5_cl_core_srv_bind( mo_action->mo_app ).
    result = lo_bind->main( val    = z5ui5_cl_util=>conv_get_as_data_ref( val )
  ...
  METHOD z5ui5_if_client~_event.
    ...
    DATA(lo_ui5) = NEW z5ui5_cl_core_srv_event( ).
    ...
    IF r_data IS NOT INITIAL.
      ...
      mo_action->ms_next-r_data = z5ui5_cl_util=>conv_copy_ref_data( r_data ).
    ENDIF.
  ...
  METHOD z5ui5_if_client~set_session_stateful.

    DATA(lv_check_sticky) = CAST z5ui5_if_app( mo_action->mo_app->mo_app )->check_sticky.
    ...
    IF stateful = abap_true.
      ...
      CAST z5ui5_if_app( mo_action->mo_app->mo_app )->check_sticky = abap_true.
    ELSE.
      mo_action->ms_next-s_set-s_stateful-active = 0.
      CAST z5ui5_if_app( mo_action->mo_app->mo_app )->check_sticky = abap_false.
    ENDIF.
    mo_action->ms_next-s_set-s_stateful-switched = abap_true.

  ENDMETHOD.

  METHOD z5ui5_if_client~check_app_prev_stack.

    DATA(ls_get) = z5ui5_if_client~get( ).
    result = xsdbool( ls_get-s_draft-id_prev_app_stack IS NOT INITIAL ).

  ENDMETHOD.

  METHOD z5ui5_if_client~check_on_init.

    result = xsdbool( CAST z5ui5_if_app(  mo_action->mo_app->mo_app )->check_initialized = abap_false ).

  ENDMETHOD.

  METHOD z5ui5_if_client~check_on_navigated.
  ...
  METHOD z5ui5_if_client~get_app_prev.

    DATA(ls_get) = z5ui5_if_client~get( ).
    result = z5ui5_if_client~get_app( ls_get-s_draft-id_prev_app ).
