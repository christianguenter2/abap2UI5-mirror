...
CLASS z2ui6_cl_ui5_srv_bind DEFINITION LOCAL FRIENDS ltcl_00_base ltcl_01_path ltcl_02_cell ltcl_03_options.
...
CLASS ltcl_00_base DEFINITION ABSTRACT
  ...
  PROTECTED SECTION.
    ...
    DATA mo_bind TYPE REF TO z2ui6_cl_ui5_srv_bind.

    " main( ) on the same service - what one render's _bind( ) calls do
    METHODS bind
      IMPORTING
        ir_val        TYPE REF TO data
        is_config     TYPE z2ui6_if_ui5_types=>ty_s_bind_config OPTIONAL
      RETURNING
    ...
    METHODS expect_bind_error
      IMPORTING
        ir_val    TYPE REF TO data
        is_config TYPE z2ui6_if_ui5_types=>ty_s_bind_config
        iv_text   TYPE string.
...
CLASS ltcl_02_cell IMPLEMENTATION.
  ...
  METHOD cell_carries_options.
    ...
    DATA(lo_mapper) = z2ui6_cl_ajson_mapping=>create_upper_case( ).
    ...
    expect_bind_error( ir_val    = cell_name( 1 )
                       is_config = VALUE #( tab           = REF #( mo_app->mt_tab )
                                            tab_index     = 1
                                            custom_mapper = z2ui6_cl_ajson_mapping=>create_lower_case( ) )
...
CLASS ltcl_03_options IMPLEMENTATION.
  ...
  METHOD later_bind_adopts.
    ...
    bind( ir_val    = REF #( mo_app->mv_value )
          is_config = VALUE #( custom_filter = lo_filter ) ).
    bind( ir_val    = REF #( mo_app->mv_value )
          is_config = VALUE #( custom_mapper = z2ui6_cl_ajson_mapping=>create_upper_case( ) ) ).
    bind( ir_val    = REF #( mo_app->mv_value )
  ...
  METHOD later_plain_bind_keeps.
    ...
    bind( ir_val    = REF #( mo_app->mv_value )
          is_config = VALUE #( custom_filter = lo_filter
                               custom_mapper = z2ui6_cl_ajson_mapping=>create_upper_case( )
                               check_json    = abap_true ) ).
  ...
  METHOD different_mapper_refused.
    ...
    expect_bind_error( ir_val    = REF #( mo_app->mv_value )
                       is_config = VALUE #( custom_mapper = z2ui6_cl_ajson_mapping=>create_lower_case( ) )
                       iv_text   = `Two different mappers` ).
  ...
  METHOD different_filter_refused.
    ...
    expect_bind_error( ir_val    = REF #( mo_app->mv_value )
                       is_config = VALUE #( custom_filter = z2ui6_cl_ajson_filter_lib=>create_empty_filter( ) )
