CLASS z2ui6_cl_ui5_srv_draft DEFINITION PUBLIC FINAL.
...
CLASS z2ui6_cl_ui5_srv_draft IMPLEMENTATION.
  ...
  METHOD cleanup.
    ...
    DATA(ls_config) = VALUE z2ui6_if_ui5_exit=>ty_s_http_config_post( ).
    ...
    DATA(lv_n_hours_ago) = z2ui6_cl_ui5_util_context=>time_subtract_seconds(
                               time    = z2ui6_cl_ui5_util_context=>time_get_timestampl( )
                               seconds = c_seconds_per_hour * ls_config-draft_exp_time_in_hours ).

    DELETE FROM z2ui6_t_01 WHERE timestampl < @lv_n_hours_ago ##SUBRC_OK.
    COMMIT WORK.
  ...
  METHOD create.
    ...
    DATA(ls_db) = VALUE ty_s_db( id                = draft-id
                                 ...
                                 uname             = sy-uname
                                 timestampl        = z2ui6_cl_ui5_util_context=>time_get_timestampl( )
                                 data              = model_xml ).
    ...
    INSERT z2ui6_t_01 FROM @ls_db.
    IF sy-subrc <> 0.
      SELECT SINGLE uname FROM z2ui6_t_01
        WHERE id = @ls_db-id
        INTO @DATA(lv_owner).
  ...
  METHOD check_exists.
    ...
    SELECT SINGLE id, uname FROM z2ui6_t_01
      WHERE id = @id
      INTO @DATA(ls_row).
  ...
  METHOD count_entries_total.
    ...
    SELECT COUNT( * ) FROM z2ui6_t_01                     "#EC CI_NOWHERE
      INTO @result.
